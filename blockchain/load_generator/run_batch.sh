...
for line in $(grep -v "#" ./batch_experiment_configs/experiments.txt); do
    ...
    sed -r -i 's/^workloadType: [^ ]*/workloadType: '"$loadType"'/' config.yaml
    sed -r -i 's/dataFileName: .\/readWriteExperiments\/(.*).json/dataFileName: .\/readWriteExperiments\/'"$loadSize"'.json/' config.yaml
    ...
    rm ~/tools/scripts/fetch-block/perfLogs_$latestFolder/*.json
