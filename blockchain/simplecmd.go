...
// InvokeOpen
func (setup *FabricSetup) WriteRand(account string, value string) (Duration, error) {
args: [, '1', '0'.repeat(5), '9'.repeat(9), '8', '20', '','']
	...
	for i := 1; i <= 100; i++ {
		...
		go func() {
			...
			defer wg.Done()
			fmt.Println("########## j is ", j)
			response, err := setup.client.Execute(channel.Request{ChaincodeID: setup.ChainCodeID, Fcn: args[0], Args: [][]byte{[]byte(strconv.Itoa(strconv.Atoi(args[1])+j)), []byte(args[2]), []byte(args[3]), []byte(args[4]), []byte(args[5]), []byte(args[6]), []byte(args[7]), []byte(args[8])}, TransientMap: transientDataMap})
			if err != nil {
				return "", fmt.Errorf("failed to randomly write: %v", err)
			}
			c1<-response
		}
	}
